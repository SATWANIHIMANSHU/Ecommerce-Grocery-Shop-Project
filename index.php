...
<body>
	...
	<div class="header-bot">
		<div class="header-bot_inner_wthreeinfo_header_mid">
			...
			<div class="col-md-8 header">
				<!-- header lists -->
				<ul class="showdeliverymodal">
				<li class="showdeliverymodal-li">
				<a href="#" id="deliveryToLink"><span class="fa fa-map-marker" aria-hidden="true"></span> Stores <span id="pincodeAddress"></span></a>
			...
			<div class="side-bar col-md-3">
				...
				<div class="left-side">
					...
					<ul>
						<li>
							<input type="checkbox" class="checked">
							<span class="span">Fortune</span>
						</li>
						<li>
							<input type="checkbox" class="checked">
							<span class="span">Dabur</span>
						</li>
						<li>
							<input type="checkbox" class="checked">
							<span class="span">Godrej</span>
						</li>
						<li>
							<input type="checkbox" class="checked">
							<span class="span">India Gate</span>
						</li>
						<li>
							<input type="checkbox" class="checked">
							<span class="span">Patanjali</span>
						</li>
						<li>
							<input type="checkbox" class="checked">
							<span class="span">Nestle</span>
						</li>
						<li>
							<input type="checkbox" class="checked">
							<span class="span"> Haldiram</span>
						</li>
						<li>
							<input type="checkbox" class="checked">
							<span class="span">Amul</span>
						</li>
						<li>
							<input type="checkbox" class="checked">
							<span class="span">Hindustan Unilever</span>
						</li>
						<li>
							<input type="checkbox" class="checked">
							<span class="span"> ITC </span>
						</li>
					</ul>
				</div>
				<!-- //cuisine -->
    ...
    <div class="row">
        ...
        $query = "SELECT * FROM products_main WHERE page = 'home'";
        $result = $conn->query($query);

        // Counter to track the number of products displayed
        $counter = 0;

        // Loop through products and display them dynamically
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                // Start a new product-sec1 div if counter is divisible by 3
                if ($counter % 3 == 0 && $counter != 0) {
                    echo '</div></div><div class="product-sec1"><div class="row">';
                }
                ?>
                    ...
                    <div class="men-pro-item simpleCart_shelfItem">
                        <form action="managecart.php" method="POST">
                            <div class="men-thumb-item">
                                <img src="<?php echo $row['image']; ?>" alt="Product Image" class="card-img-top img-fluid" style="max-height: 150px; object-fit: cover;">
                            ...
                            <div class="item-info-product">
                                ...
                                <div class="info-product-price">
                                    ...
                                    <?php if ($row['old_price']) { ?>
                                        <del>₹<?php echo $row['old_price']; ?></del>
                                    <?php } ?>
                                </div>
...
subscribeForm.addEventListener('submit', function (event) {
    ...
    subscribeForm.reset(); // Clear the input fields
});
...
window.addEventListener('click', function (event) {
	...
	<script>
		...
		<!-- show delivery modal script  -->
	
		<script>
	 function fetchAddress(pincode) {
  ...
  fetch(apiUrl)
    ...
    .then(data => {
      if (data[0].Status === "Success" && data[0].PostOffice) {
        ...
        const address = `${postOffice.Name}, ${postOffice.State}`;

        // Update navbar with the address
        document.getElementById('pincodeAddress').textContent = address;

        // Save city to the database
        saveCityToDatabase(postOffice.Name, postOffice.State);
...
function saveCityToDatabase(city, state) {
  fetch('save_pincode.php', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/x-www-form-urlencoded',
    },
    body: `city=${encodeURIComponent(city)}&state=${encodeURIComponent(state)}`
  })
    .then(response => response.json())
    .then(data => {
      if (data.status === 'success') {
        ...
        console.error('Error saving city:', data.message);
      }
    })
    .catch(error => {
